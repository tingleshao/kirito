...
#adjacent_map = [[1, 11, 9],[0, 2],[1, 3],[2, 4],[5],[4, 6],[5, 7],[6, 8],[7, 9],[8, 10],
#                [9, 11],[0, 10, 12],[11, 13],[12, 14],[13, 15],[14, 16],[15, 17],[4, 16]]
...
adjacent_map = [[1, 8, 9, 10, 11, 12, 13, 14],[0, 2, 7, 8, 9, 13, 14, 15],[1, 3, 6, 7, 8, 14, 15, 16],
                [2, 4, 5, 6, 7, 15, 16, 17],[5, 6, 16, 17],[6],[7],[8],[9],[10],
...
#    ratio = 920.0 / 670.0
    ...
    #         curr_i = curr_i + 1
    #     curr_idx = curr_i
    #     if curr_key[1] in adjacent_map2[curr_key[0]]:
    #         important_features = [values_lst[w] for w in sorted(range(len(values_lst)), key=lambda k: dist_lst[k])]
    #         for i in range(min(len(important_features), 25)):
    #             output_str = output_str + "c n{0} N{1} x{2:.12f} y{3:.12f} X{4:.12f} Y{5:.12f} t0\n".format(curr_key[0], curr_key[1], important_features[i][0] * ratio, important_features[i][1] * ratio, important_features[i][2] * ratio, important_features[i][3] * ratio)
    ...
    #         curr_i = curr_i + 1
    #     curr_idx = curr_i
    #     if curr_key[1] in adjacent_map3[curr_key[0]]:
    #         important_features = [values_lst[w] for w in sorted(range(len(values_lst)), key=lambda k: dist_lst[k])]
    #         for i in range(min(len(important_features), 25)):
    #             output_str = output_str + "c n{0} N{1} x{2:.12f} y{3:.12f} X{4:.12f} Y{5:.12f} t0\n".format(curr_key[0], curr_key[1], important_features[i][0] * ratio, important_features[i][1] * ratio, important_features[i][2] * ratio, important_features[i][3] * ratio)
